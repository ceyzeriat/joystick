...
class GraphMulti(Graph):
    def __init__(self, name, freq_up=1, pos=(50, 50), size=(400, 400),
        ...
        Args:
          ...
          * lbls (list of str) [optional]: the labels to display in the
            legend. Default ``None`` is equivalent to ``['L1', ..., 'Ln']``
        ...
        Kwargs:
          * Any parameter accepted by ``figure.add_axes`` (eg. ``xlabel``,
            ``ylabel``, ``title``, aspect``)
    ...
    def _add_text(self, ith, x=None, y=None):
        ...
        self.ax.text(x, y, str(ith+1), color='w', bbox=dict(color='k', alpha=0.5))

    @property
    def lbls(self):
        """
        The labels for the legend (len=:py:func:`~joystick.graph.GraphMulti.nlines`)
        Set to ``None`` to simply display ('L1', ..., 'Ln').
        """
        return self._lbls
    ...
    def legend(self, show, lbls=[], loc=None, **kwargs):
        ...
        Kwargs:
          * Any parameter accepted by ``figure.ax.legend``
        """
        show = bool(show)
        if show:
            if lbls != []:
    ...
    def set_xydata(self, x, y, ln=None):
        ...
        If ``ln`` is left ``None``, x and y are expected to be lists
        (len=:py:func:`~joystick.graph.GraphMulti.nlines`) of numpy 1d-vectors.
        Otherwise ``ln`` shall be the integer line index to update; x and y
        shall be numpy 1d-vectors.
        Only the last :py:func:`~joystick.graph.GraphMulti.xnpts` data-points
        will be displayed
        """
        if not self.visible:
            return
    ...
    def _set_data_and_text(self, ith, x, y):
        ...
        if self.numbering:
            idx = -self.xnpts if x.size >= self.xnpts else 0
            self.ax.texts[ith].set_position((x[idx], y[idx]))
    
    def get_xydata(self, ln=None):
        """
        Returns the x and y data from the graph: ([x1,x2,...], [y1,y2,...])
        unless ``ln`` is not ``None``
        """
        if ln is not None:
            return self.ax.lines[int(ln)].get_xdata(), self.ax.lines[int(ln)].get_ydata()
        else:
